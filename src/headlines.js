...
const CH = {
  launch: {
    label: "新车上市 · 销量",
    focus: "新车/改款上市、预售与定价、订单(小定/大定)、月度销量与交付榜",
    queries: ["新能源汽车 新车 上市 发布 定价", "汽车 新车 预售 大定 小定 订单", "车企 月度 销量 交付 榜单", "新款 改款 车型 上市", "热门 车型 销量 环比"],
    gnews: ["新能源汽车 上市 发布", "车企 销量 交付", "新车 预售 定价"],
  },
  fin: {
    label: "财务 · 融资",
...
export async function genHeadlines(channel) {
  ...
  const { cn } = today();
  const schema = `今天是 ${cn}。请基于下方资料整理中国汽车行业「${c.label}」方向的今日要闻,聚焦:${c.focus}。要求:① 只选最近 2-3 天发布的新闻,按发布时间从新到旧;② 同一事件只列一次,合并重复;③ 给 8-12 条,每条配一段 2-3 句的客观摘要 summary;④ url 必须取自资料中真实出现的链接,不可编造。JSON:{"date":"${cn}","items":[{"title":"标题","summary":"2-3句客观摘要","source":"来源媒体","url":"真实URL","time":"发布日期如8月28日"}]}`;
