...
export async function updateFromNews(newsData) {
  ...
  const ctx = items.map((it, i) => `[${i + 1}] ${it.title || ""}\n摘要: ${String(it.summary || "").slice(0, 200)}`).join("\n\n");
  const prompt = `下面是今天的中国汽车行业新闻。请只提取其中**对具体车型的明确变化**(新车上市、改款上市、价格调整、智驾/配置重大变化、状态变化如开启预售/正式上市),严格依据新闻明说的内容,不要推测、不要泛泛而谈。每条:
{brand:品牌, model:车型名(尽量规范完整), body:轿车/SUV/MPV(若提及), priceFrom:起售价数字万(若新闻给了定价/调价), priceRange:价格区间(若提及), adas:智驾系统(若提及), hi:是否搭载华为乾崑(true/false,仅在明确时给), status:状态(若提及如"上市"/"预售"), launch:{kind:"新车"或"改款", month:1-12, date:"如3月", note:"亮点20字内"}(仅当是上市/改款时给), note:"一句话(30字内)"}
没有任何明确的具体车型变化时,返回 {"deltas":[]}。不要编造品牌或车型。
JSON:{"deltas":[{"brand":"","model":"","status":"","launch":{"kind":"新车","month":6,"date":"6月","note":""}}]}
