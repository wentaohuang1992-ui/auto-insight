...
和 `fin_review.js` 的分工：**review 是单家深度**（11 条风险信号 + 对我们的含义），
给要做客户决策时看；**flash 是横向速览**（披露日期 + 一句话总结），
给财报季扫一眼谁出了、出了什么。

## 覆盖 17 家：整车 12 + 零部件供应商 5

整车：比亚迪 上汽 吉利 长安 奇瑞 长城 广汽 赛力斯 理想 零跑 小鹏 蔚来
供应商：地平线(09660.HK) 德赛西威(002920.SZ) 禾赛科技(02525.HK) 速腾聚创(02498.HK) 承泰科技(未上市)

**本模块不依赖 `fin_db` 的 quarterly 表**，自己按股票代码去取 —— 所以供应商
（不在预置的 15 家车企里）也能进这张表。名单存在 `flash.json` 的 `entities`，
可以在接口里增删改，不用改代码。

`verified:false` 的 5 个代码（奇瑞/零跑/地平线/禾赛/速腾）是按公开信息填的、没实跑验证过，
首次抓取失败会在该行的 `errors` 里点名，到时候改一下代码即可。
承泰科技未上市，拿不到结构化报表，总结只能来自检索。

## 三段数据来源分得很清楚

| 段 | 来源 | 用不用模型 |
| --- | --- | --- |
| ① 发布时间 | 东方财富 F10 的 **`NOTICE_DATE`** 字段 | 不用 |
| ② 核心数字 | 同一接口的营收/归母净利及同比，累计口径（年报=全年，一季报=Q1） | 不用 |
| ③ 总结正文 | 博查检索该期财报报道 → DeepSeek 依据①②③写 2-4 句 | 用 |

数字校验：总结里出现的每个数字，必须能在「核心数字」或「检索资料原文」里找到。
找不到判幻觉、带着错处重试一次；仍不过退回只有核心数字的模板句（`mode: "template"`）。

## 实测：结构化那半完全对得上

拿比亚迪跑（把真实接口响应做成本地桩，验证字段解析与期间匹配）：

| | 发布时间 | 营收 | 归母净利 |
| --- | --- | --- | --- |
| 2025年财报 | 2026-03-28 | 8039.65 亿（+3.46%） | 326.19 亿（-18.97%） |
| 2026年一季度报告 | 2026-04-29 | 1502.25 亿（-11.82%） | 40.85 亿（-55.38%） |

数字与需求里给的样例逐位一致。**发布时间差一天**（样例写 3/27、4/28，接口是 3/28、4/29）——
接口取的是交易所正式披露日 `NOTICE_DATE`，样例大概是媒体报道日，以接口为准。

降级路径也验证过：容器无外网时返回
`A股(002594.SZ): RPT_F10_FINANCE_GINCOME HTTP 403`，行照样生成、进程不挂；
承泰科技这种无代码的返回「未上市或未登记股票代码,拿不到结构化报表」。

## 接口

| 接口 | 说明 |
| --- | --- |
| `GET /api/fin/flash` | 整表（名单 + 报告期 + 已生成的行） |
| `POST /api/fin/flash` `{company}` | 生成一家 |
| `POST /api/fin/flash` `{group:"整车"\|"供应商"}` | 按组生成 |
| `POST /api/fin/flash` `{}` | 全部 17 家 |
| `POST /api/fin/flash/entities` / `PUT .../:id` / `DELETE .../:id` | 名单增删改 |
| `PUT /api/fin/flash/periods` | 换报告期（比如加「2026年中报」） |
| `POST /api/refresh` `{what:"fin-flash"}` | 全量（另有 `fin-flash-oem` / `fin-flash-sup`） |

前端：财报板块顶部新增「财报速递」标签，按整车/供应商分组渲染，
每行末尾有单独的「生成/↻」按钮，工具条上有「全部生成 / 只跑整车 / 只跑供应商」。

## 新增/修改文件

| 文件 | 动作 |
| --- | --- |
| `src/fin_flash.js` | 新增（数据文件 `flash.json`，走 store.js） |
| `server.js` | 加 6 条路由 + 3 个全量任务 |
| `public/fin-board.js` | 新增「财报速递」标签页；`pollJob` 加 `onDone` 回调 |

## 遗留

- 港股的 `NOTICE_DATE` 在长表里未必有，取不到就显示「未取到」，不编。
- 一次全量 = 17 家 × (1 轮接口 + 3 个检索查询 + 1 次 LLM)。**建议按组跑**，
  别一次全量把博查额度打光；`overBudget()` 会兜底但那是硬拦。
- `EM_API_BASE` 环境变量可覆盖东方财富接口地址（换域名时不用改代码，也便于造桩测试）。

## 追加（同日）：加 2026 中报 + 表结构改成一期一列

**加 2026 年半年报**。默认报告期变成三个：2025年财报 / 2026年一季度报告 / 2026年半年报。
顺带修了一个隐患：原来算期末日是「先试 31，不行试 30」，6 月和 9 月靠 fallback 撞对；
现在改成查表 `{03:31, 06:30, 09:30, 12:31}`。

**区分「尚未披露」和「取数失败」**。八月正是中报披露期，一半公司还没发。
以前两种情况都显示成一个「—」，看不出到底是没发还是抓挂了。现在分三种状态：

| 状态 | 含义 |
| --- | --- |
| `ok` | 取到了 |
| `未披露` | 接口通了但没有这一期 —— 财报季进行中，这家还没发 |
| `取数失败` | 接口没通（代码错 / 被限流 / 域名变了），该行会用红字给出错误原文 |
| `无结构化源` | 未上市（承泰科技），只能靠检索 |

**表结构：一个报告期一列。** 原来「发布时间」「总结」各占一列，3 个报告期就是 7 列，
横向滚不完。改成一期一列，发布时间作为小字标在总结上方；「生成」按钮也挪进车企那一格。
4 列，1400px 宽度内放得下。

**老数据自动升级**：`ensureSeeded()` 里判断，只有在「没自定义过报告期」（存的是默认集合的
子集）时才自动补上新的默认期；你手工改过 `periods` 的话原样保留，不动。

实测（本地桩）：比亚迪三期都 ok；德赛西威桩里没有 2026-06-30 → 标「尚未披露」；
承泰科技无代码 → 三期都是「无结构化源」。
